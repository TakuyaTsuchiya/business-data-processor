...
def process_faith_sms_guarantor_data(file_content: bytes, payment_deadline_date: date) -> ProcessingResult:
    """
    フェイスSMS保証人データ処理（型安全版）
    ...
    try:
        ...
        # AU列（列番号46）の電話番号を取得
        guarantor_phone_series = df.iloc[:, 46].astype(str).str.strip().replace('nan', '')
        ...
        # 除外データの詳細を記録
        excluded_phones_mask = ~guarantor_phone_series.apply(is_mobile_phone)
        if excluded_phones_mask.sum() > 0:
            invalid_phone_samples = guarantor_phone_series[excluded_phones_mask].value_counts().head(5).to_dict()
            logs.append(f"フィルター5除外 - AU列TEL携帯形式例: {invalid_phone_samples}")
        
        df = df[guarantor_phone_series.apply(is_mobile_phone)]
        ...
        output_filename = f"{date_str}フェイスSMS保証人.csv"
        ...
        result = ProcessingResult(
            ...
            messages=logs,
            processor_name="フェイスSMS保証人",
            metadata={"output_filename": output_filename}
        )
        
        logger.info(f"フェイスSMS保証人処理完了: {len(df_copy)}件を処理")
        return result
        
    except Exception as e:
        # エラー時のProcessingResult作成
        processing_time = time.time() - start_time
        logger.error(f"フェイスSMS保証人処理エラー: {str(e)}")
        ...
        return ProcessingResult(
            ...
            errors=[f"FAITH SMS保証人処理エラー: {str(e)}"],
            processor_name="フェイスSMS保証人"
        )