...
def process_faith_sms_emergency_contact_data(file_content: bytes, payment_deadline_date: date) -> ProcessingResult:
    ...
    try:
        ...
        # Filter 5: 緊急連絡人１のTEL（携帯） (Keep only valid mobile phone numbers)
        mobile_phone_regex = r'^(090|080|070)-\d{4}-\d{4}$'
        
        # 緊急連絡人１のTEL（携帯）列の電話番号を取得
        emergency_phone_series = df['緊急連絡人１のTEL（携帯）'].astype(str).str.strip().replace('nan', '')
        ...
        if excluded_phones_mask.sum() > 0:
            ...
            logs.append(f"フィルター5除外 - 緊急連絡人１のTEL（携帯）形式例: {invalid_phone_samples}")
        
        df = df[emergency_phone_series.apply(is_mobile_phone)]
        logs.append(f"フィルター5 - 緊急連絡人１のTEL（携帯）(090/080/070のみ): {len(df)}件")
        ...
        # Map data - 緊急連絡人１のTEL（携帯）を使用
        output_df['電話番号'] = df['緊急連絡人１のTEL（携帯）'].astype(str)
