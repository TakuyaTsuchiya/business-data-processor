...
def process_plaza_sms_contact_data(file_content: bytes, payment_deadline_date: date) -> Tuple[pd.DataFrame, List[str], str, dict]:
    ...
    try:
        ...
        if excluded_phones_mask.sum() > 0:
            invalid_phone_samples = contact_phone_series[excluded_phones_mask].value_counts().head(5).to_dict()
            logs.append(f"フィルター5除外 - BE列緊急連絡人１TEL形式例: {invalid_phone_samples}")
        
        df = df[contact_phone_series.apply(is_mobile_phone)]
        logs.append(f"フィルター5 - BE列緊急連絡人１TEL(090/080/070のみ): {len(df)}件")
