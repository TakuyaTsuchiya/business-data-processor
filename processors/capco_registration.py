...
class CapcoConfig:
    ...
    # 支店コードマッピング
    BRANCH_CODE_MAPPING = {"中央支店": "763", "東海支店": "730"}

    # クライアントCDマッピング
    CLIENT_CD_MAPPING = {"1004": "1", "1005": "4"}
...
class DuplicateChecker:
    ...
    def find_new_contracts(
        self, capco_df: pd.DataFrame, contract_df: pd.DataFrame
        ...
        # 新規・既存を分離
        new_contracts = capco_df[
            ~capco_df["契約No"].isin(existing_contract_numbers)
        ].copy()
        existing_contracts = capco_df[
            capco_df["契約No"].isin(existing_contract_numbers)
        ].copy()
        ...
        stats = {
            ...
            "new_percentage": (len(new_contracts) / len(capco_df) * 100)
            if len(capco_df) > 0
            else 0,
        }
...
class DataConverter:
    ...
    def split_address(
        self, full_address: str, building_name: str, room_name: str
    ) -> Dict[str, str]:
        """住所分割処理（辞書ベースAddressSplitter使用）"""
        ...
        # AddressSplitterで都道府県・市区町村を抽出
        result = self.address_splitter.split_address(str(full_address).strip())

        # 建物名・部屋名の組み立て
        building = str(building_name).strip() if pd.notna(building_name) else ""
        room = str(room_name).strip() if pd.notna(room_name) else ""

        # 現住所3組み立て（remaining + 建物名 + 部屋名）
        address3_parts = []
        if result["remaining"].strip():
            address3_parts.append(result["remaining"].strip())
        if building:
        ...
        property_address3 = result["remaining"].strip()
    ...
    def convert_new_contracts(self, new_contracts_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, row in new_contracts_df.iterrows():
            ...
            converted_row["契約者カナ"] = self.convert_kana(
                row.get("契約者ふりがな", "")
            )

            # 2. 電話番号処理
            home_tel, mobile_tel = self.process_phone_numbers(
                row.get("契約者：電話番号", ""), row.get("契約者：携帯番号", "")
            )
            ...
            address_parts = self.split_address(
                row.get("建物：住所", ""), row.get("建物名", ""), row.get("部屋名", "")
            )

            # 契約者現住所
            converted_row["契約者現住所郵便番号"] = str(
                row.get("建物：郵便番号", "")
            ).strip()
            ...
            # 6. 口座情報
            converted_row["回収口座金融機関名"] = str(
                row.get("V口座銀行名", "")
            ).strip()
            converted_row["回収口座支店CD"] = self.get_branch_code(
                row.get("V口振支店名", "")
            )
            ...
            if (
                pd.isna(management_company)
                or str(management_company).strip().lower() == "nan"
            ):
                management_company = ""
            else:
...
def process_capco_data(
    capco_content: bytes, contract_content: bytes
    ...
    try:
        ...
        logs.append(
            f"ファイル読み込み完了: カプコ{len(capco_df)}件, ContractList{len(contract_df)}件"
        )

        # 2. 重複チェック（新規案件抽出）
        duplicate_checker = DuplicateChecker()
        new_contracts, existing_contracts, match_stats, detail_logs = (
            duplicate_checker.find_new_contracts(capco_df, contract_df)
        )
