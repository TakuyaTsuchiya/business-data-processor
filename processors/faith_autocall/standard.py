...
def apply_filters(df_input: pd.DataFrame) -> Tuple[pd.DataFrame, list]:
    ...
    df = df_input.copy()
    logs = []
    filter_conditions = FaithConfig.FILTER_CONDITIONS
    
    initial_count = len(df)
    logs.append(f"初期データ件数: {initial_count}件")
    
    # 委託先法人IDのフィルタリング（1,2,3,4のみ）
    if "委託先法人ID_in" in filter_conditions:
        df["委託先法人ID"] = pd.to_numeric(df["委託先法人ID"], errors="coerce")
        df = df[df["委託先法人ID"].isin(filter_conditions["委託先法人ID_in"])]
        logs.append(f"委託先法人IDフィルタリング後: {len(df)}件")
    
    # 入金予定日のフィルタリング（NaNか昨日以前）
    ...
    # 回収ランクのフィルタリング（死亡決定、破産決定、弁護士介入を除外）
...
def map_data_to_template(df_filtered: pd.DataFrame) -> pd.DataFrame:
    ...
    mapping_rules = FaithConfig.MAPPING_RULES
    ...
    if "委託先法人ID" in df_filtered.columns:
        valid_ids = df_filtered["委託先法人ID"].dropna()
        if not valid_ids.empty:
            df_template["クライアント"] = [
                f"フェイス{int(id)}" for id in df_filtered["委託先法人ID"].values 
                if pd.notna(id)
            ]
        else:
            df_template["クライアント"] = ""
...
def process_faith_data(file_content: bytes) -> Tuple[pd.DataFrame, pd.DataFrame, list, str]:
    """
    フェイス契約者データの処理メイン関数
    ...
    try:
        ...
        output_filename = f"{today_str}{FaithConfig.OUTPUT_FILE_PREFIX}.csv"
        
        return df_filtered, df_output, logs, output_filename
        
    except Exception as e:
        raise Exception(f"フェイスデータ処理エラー: {str(e)}")
