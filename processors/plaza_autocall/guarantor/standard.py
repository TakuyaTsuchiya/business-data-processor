...
def apply_plaza_guarantor_filters(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    """
    ...
    # 5. 特殊残債・クライアントCDのフィルタリング（with10k版では除外なし - 全件処理）
...
def process_plaza_guarantor_data(file_content: bytes) -> Tuple[pd.DataFrame, pd.DataFrame, List[str], str]:
    """
    プラザ保証人データの処理メイン関数
    ...
    try:
        ...
        # 2. 保証人フィルタリング処理
        df_filtered, filter_logs = apply_plaza_guarantor_filters(df_input)
