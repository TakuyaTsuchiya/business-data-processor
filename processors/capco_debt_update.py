...
CONTRACT_USECOLS = [
    ...
    71,
    97,
]  # 管理番号(A列), 引継番号(B列), 滞納残債(BT列), クライアントCD(CT列)
...
def detect_encoding(file_content: bytes) -> str:
    """ファイルのエンコーディングを自動検出する（先頭10KBのみサンプリング）"""
    # 大きなファイルの場合、先頭10KBのみをサンプリングしてエンコーディングを検出
...
def read_csv_with_encoding(
    file_content: bytes, file_name: str, usecols: Optional[list] = None
) -> pd.DataFrame:
...
def extract_arrear_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(
            f"滞納データの読み込み列数: {len(df.columns)} 列（メモリ最適化済み）"
        )
        ...
        logger.info(
            f"重複削除: {before_count} -> {after_count} 件 (削除: {before_count - after_count} 件)"
        )
...
def extract_contract_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(
            f"ContractListの読み込み列数: {len(df.columns)} 列（メモリ最適化済み）"
        )
...
def merge_data(contract_data: pd.DataFrame, arrear_data: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        contract_data["引継番号"] = (
            contract_data["引継番号"].astype(str).str.replace(".0", "", regex=False)
        )
        ...
        logger.info(
            f"引継番号サンプル（10件）: {contract_data['引継番号'].head(10).tolist()}"
        )
        logger.info(f"引継番号の実際の型: {type(contract_data['引継番号'].iloc[0])}")
        logger.info(
            f"契約Noサンプル（10件）: {arrear_data['契約No'].head(10).tolist()}"
        )
        ...
        if common_values:
            ...
            # サンプル比較
            logger.info(
                f"引継番号の例（ソート済み最初の5件）: {sorted(list(contract_set))[:5]}"
            )
            logger.info(
                f"契約Noの例（ソート済み最初の5件）: {sorted(list(arrear_set))[:5]}"
            )
        ...
        merged_df.loc[mask, "滞納額合計_num"] = pd.to_numeric(
            merged_df.loc[mask, "滞納額合計"].map(clean_amount), errors="coerce"
        )
        merged_df["滞納残債_num"] = pd.to_numeric(
            merged_df["滞納残債"].map(clean_amount), errors="coerce"
        )
...
def extract_changed_data(merged_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(
            f"滞納額合計_num有効: {merged_df['滞納額合計_num'].notna().sum()} 件"
        )
        ...
        completed_payments = unmatched[
            (unmatched["滞納残債_num"] > 0) & unmatched["滞納残債_num"].notna()
        ].copy()
        ...
        if len(completed_payments) > 0:
            changed_df = pd.concat(
                [matched_changed, completed_payments], ignore_index=True
            )
        else:
            changed_df = matched_changed

        logger.info(
            f"差分抽出結果（完済含む）: {len(merged_df)} -> {len(changed_df)} 件"
        )
        logger.info(f"  - マッチして差分あり: {len(matched_changed)} 件")
        ...
        if len(changed_df) > 0:
            if "__delta__" not in changed_df.columns:
                changed_df["__delta__"] = (
                    changed_df["滞納額合計_num"] - changed_df["滞納残債_num"]
                )
        ...
        output_df = pd.DataFrame(
            {
                ...
                OUTPUT_HEADERS[1]: changed_df["滞納額合計_num"]
                .fillna(0)
                .round(0)
                .astype("Int64"),  # 管理前滞納額
            }
...
def process_capco_debt_update(
    arrear_file_content: bytes, contract_file_content: bytes, progress_callback=None
) -> Tuple[pd.DataFrame, str, Dict[str, any], List[str]]:
    """
    カプコ残債更新処理のメイン関数
    ...
    try:
        ...
        arrear_df = read_csv_with_encoding(
            arrear_file_content, "csv_arrear_*.csv", usecols=ARREAR_USECOLS
        )
        contract_df = read_csv_with_encoding(
            contract_file_content, "ContractList_*.csv", usecols=CONTRACT_USECOLS
        )
        ...
        logger.info(
            f"メモリ最適化完了 - 滞納データ: {stats['arrear_columns_optimized']}列読み込み"
        )
        logger.info(
            f"メモリ最適化完了 - ContractList: {stats['contract_columns_optimized']}列読み込み"
        )
        ...
        stats["arrear_duplicates_removed"] = (
            stats["arrear_unique_before"] - stats["arrear_unique_after"]
        )
        stats["contract_extracted"] = len(contract_data)
        stats["contract_columns"] = len(
            contract_df.columns
        )  # メモリ最適化後の列数（4列）
        ...
        stats["client_cd_excluded"] = (
            stats["client_cd_before"] - stats["client_cd_after"]
        )
        ...
        # 除外されたクライアントCDの詳細
        excluded_client_cd = contract_data[
            ~contract_data["管理番号"].isin(contract_filtered["管理番号"])
        ]
        ...
        stats["diff_unchanged"] = stats["diff_valid"] - (
            stats["diff_changed"] - completed_count
        )
