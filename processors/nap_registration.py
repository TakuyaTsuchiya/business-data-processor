...
class DataMapper:
    ...
    def map_guarantor_info(
        ...
        if "連帯保証人関係" in excel_df.columns:
            # 値がある場合は全て「他」に変更
            relation = excel_df["連帯保証人関係"].fillna("")
            output_df["保証人１契約者との関係"] = relation.where(relation == "", "他")
    ...
    def map_emergency_contact_info(
        ...
        if "緊急連絡人関係" in excel_df.columns:
            # 値がある場合は全て「他」に変更
            relation = excel_df["緊急連絡人関係"].fillna("")
            output_df["緊急連絡人１契約者との関係"] = relation.where(relation == "", "他")
