"""
ミライル緊急連絡人オートコール処理プロセッサー（残債10,000円・11,000円除外・リファクタリング版）
共通フィルタリングエンジンを使用
"""
...
class MirailEmergencyContactWithout10kConfig:
    """ミライル緊急連絡人（残債除外版）処理の設定"""
    ...
    FILTER_CONFIG = {
        ...
        "mobile_phone": {
            "column": COL.TEL_MOBILE_2,  # 緊急連絡人電話番号（BE列=列56）
            "log_type": "phone",
            "label": "緊急連絡人電話"
        },
        "payment_amount": {
    ...
    MAPPING_RULES = {
        ...
        "契約者名（カナ）": COL.CONTRACT_KANA,  # 緊急連絡人でも契約者名を入れる
...
def create_mirail_emergency_output(df_filtered: pd.DataFrame) -> pd.DataFrame:
    ...
    # データをマッピング
    mapping_rules = MirailEmergencyContactWithout10kConfig.MAPPING_RULES
