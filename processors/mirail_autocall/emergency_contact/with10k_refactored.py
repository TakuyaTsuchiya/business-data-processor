"""
ミライル緊急連絡人オートコール処理プロセッサー（残債含む全件・リファクタリング版）
共通フィルタリングエンジンを使用
"""
...
def create_mirail_emergency_output(df_filtered: pd.DataFrame) -> pd.DataFrame:
    ...
    # データをマッピング
    mapping_rules = MirailEmergencyContactWith10kConfig.MAPPING_RULES
...
def process_mirail_emergency_contact_with10k_data(file_content: bytes) -> Tuple[pd.DataFrame, List[str], str]:
    ...
    try:
        ...
        # 2. 共通フィルタリングエンジンを使用
        df_filtered, filter_logs = apply_filters(df_input, MirailEmergencyContactWith10kConfig.FILTER_CONFIG)
        ...
        output_filename = f"{today_str}{MirailEmergencyContactWith10kConfig.OUTPUT_FILE_PREFIX}.csv"
