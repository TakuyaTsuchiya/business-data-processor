...
def apply_filters(df_input: pd.DataFrame) -> Tuple[pd.DataFrame, list]:
    ...
    # 【追加条件1】クライアントCDフィルタリング（1,4,5,10,40のみ）
    if "クライアントCD_in" in filter_conditions:
        # 既に数値化済み
        df = df[df["クライアントCD"].isin(filter_conditions["クライアントCD_in"])]
        logs.append(f"クライアントCD(1,4,5,10,40)フィルタリング後: {len(df)}件")
    
    # 【追加条件2】滞納比率フィルタリング（1.3以上のみ）
    if "滞納比率_threshold" in filter_conditions:
        # 月額賃料合計を数値化
        ...
        # 1.3以上のレコードのみ残す（1.3未満を除外）
        threshold = filter_conditions["滞納比率_threshold"]
        df = df[df["滞納比率"] >= threshold]
        logs.append(f"滞納比率{threshold}以上フィルタリング後: {len(df)}件")
