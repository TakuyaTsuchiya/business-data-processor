"""
ミライル契約者データ処理プロセッサー（残債除外版・リファクタリング版）
共通フィルタリングエンジンを使用
"""
...
def create_mirail_output(df_filtered: pd.DataFrame) -> pd.DataFrame:
    """ミライル出力データ作成（28列統一フォーマット）"""
    ...
    # データをマッピング
    mapping_rules = MirailWithout10kConfig.MAPPING_RULES
...
def process_mirail_contract_without10k_data(file_content: bytes) -> Tuple[pd.DataFrame, List[str], str]:
    ...
    try:
        ...
        # 2. 共通フィルタリングエンジンを使用
        df_filtered, logs = apply_filters(df_input, MirailWithout10kConfig.FILTER_CONFIG)
        
        # 3. 出力データ作成
        df_output = create_mirail_output(df_filtered)
        
        # 4. 出力ファイル名生成
        today_str = datetime.now().strftime("%m%d")
        output_filename = f"{today_str}{MirailWithout10kConfig.OUTPUT_FILE_PREFIX}.csv"
        
        return df_output, logs, output_filename
        
    except Exception as e:
        raise Exception(f"ミライル契約者（残債除外版）データ処理エラー: {str(e)}")