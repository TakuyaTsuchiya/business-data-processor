"""
ミライル保証人オートコール処理プロセッサー（残債含む全件・リファクタリング版）
共通フィルタリングエンジンを使用
"""
...
def create_mirail_guarantor_output(df_filtered: pd.DataFrame) -> pd.DataFrame:
    ...
    # データをマッピング
    mapping_rules = MirailGuarantorWith10kConfig.MAPPING_RULES
...
def process_mirail_guarantor_with10k_data(file_content: bytes) -> Tuple[pd.DataFrame, List[str], str]:
    ...
    try:
        ...
        # 2. 共通フィルタリングエンジンを使用
        df_filtered, filter_logs = apply_filters(df_input, MirailGuarantorWith10kConfig.FILTER_CONFIG)
        ...
        output_filename = f"{today_str}{MirailGuarantorWith10kConfig.OUTPUT_FILE_PREFIX}.csv"
