"""
ミライル緊急連絡人オートコール処理プロセッサー（残債含む全件）
統合アプリ用に移植
"""
...
class MirailEmergencyContactWith10kConfig:
    ...
    # マッピングルール（列番号ベース）
    MAPPING_RULES = {
        "電話番号": COL.TEL_MOBILE_2,
        "架電番号": COL.TEL_MOBILE_2, 
...
def apply_mirail_emergencycontact_with10k_filters(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    """
    ミライル緊急連絡人（残債含む）フィルタリング処理
    ...
    if "回収ランク_not_in" in MirailEmergencyContactWith10kConfig.FILTER_CONDITIONS:
        before_filter = len(df)
        # 除外されるデータの詳細を記録
        excluded_data = df[df["回収ランク"].isin(MirailEmergencyContactWith10kConfig.FILTER_CONDITIONS["回収ランク_not_in"])]
        if len(excluded_data) > 0:
    ...
    if "入金予定金額_not_in" in MirailEmergencyContactWith10kConfig.FILTER_CONDITIONS:
        df["入金予定金額"] = pd.to_numeric(df["入金予定金額"], errors='coerce')
        before_filter = len(df)
        # 除外されるデータの詳細を記録
        excluded_data = df[df["入金予定金額"].isin(MirailEmergencyContactWith10kConfig.FILTER_CONDITIONS["入金予定金額_not_in"])]
        if len(excluded_data) > 0:
        ...
        df = df[df["入金予定金額"].isna() | ~df["入金予定金額"].isin(MirailEmergencyContactWith10kConfig.FILTER_CONDITIONS["入金予定金額_not_in"])]
...
def process_mirail_emergencycontact_with10k_data(file_content: bytes) -> Tuple[pd.DataFrame, pd.DataFrame, List[str], str]:
    """
    ミライル緊急連絡人（残債含む）データの処理メイン関数
    ...
    try:
        ...
        # 2. フィルタリング処理
        df_filtered, filter_logs = apply_mirail_emergencycontact_with10k_filters(df_input)
