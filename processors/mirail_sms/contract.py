...
def process_mirail_sms_contract_data(file_content: bytes, payment_deadline_date: date) -> Tuple[pd.DataFrame, List[str], str, dict]:
    ...
    try:
        ...
        # Filter 5: AB列　TEL携帯 (Keep only valid mobile phone numbers)
        # AB列は列番号27（0ベース）
        ...
        if excluded_phones_mask.sum() > 0:
            ...
            logs.append(f"フィルター5除外 - AB列TEL携帯形式例: {invalid_phone_samples}")
        
        df = df[mobile_phone_column.apply(is_mobile_phone)]
        logs.append(f"フィルター5 - AB列TEL携帯(090/080/070のみ): {len(df)}件")
