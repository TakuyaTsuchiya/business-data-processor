...
class VisitListBackrentConfig:
    """訪問リスト作成（バックレント用）の設定・定数"""
    ...
    OUTPUT_FILE_PREFIX = "訪問リスト（バックレント用）"
...
def filter_records(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    ...
    2. 回収ランク: 「交渉困難」「死亡決定」「弁護士介入」「破産決定」を除外
    3. 入金予定日: 当日以前または空白
    4. 入金予定金額: 2, 3, 5を除外
    ...
    # 1. 入居ステータスフィルタ（「入居中」「退去済」のみ）
    before_count = len(df)
    occupancy_status = df.iloc[:, ContractListColumns.OCCUPANCY_STATUS].astype(str).str.strip()
    mask_occupancy = occupancy_status.isin(["入居中", "退去済"])
    df_filtered = df[mask_occupancy].copy()
    excluded_count = before_count - len(df_filtered)
    # 残存内訳をカウント
    filtered_occupancy = df_filtered.iloc[:, ContractListColumns.OCCUPANCY_STATUS].astype(str).str.strip()
    count_occupied = (filtered_occupancy == "入居中").sum()
    ...
    # 残存内訳をカウント
    count_before_today = (df_filtered['payment_date_parsed'].dt.date <= today).sum()
    count_blank_date = df_filtered['payment_date_parsed'].isna().sum()
    df_filtered = df_filtered[mask_date].copy()
    df_filtered = df_filtered.drop(columns=['payment_date_parsed'])
    excluded_count = before_count - len(df_filtered)
    logs.append(f"入金予定日フィルタ: {excluded_count}件除外 → {len(df_filtered)}件残存（当日以前: {count_before_today}件, 空白: {count_blank_date}件）")

    # 4. 入金予定金額フィルタ（2, 3, 5を除外）
    before_count = len(df_filtered)
    payment_amount_numeric = pd.to_numeric(df_filtered.iloc[:, ContractListColumns.PAYMENT_DUE_AMOUNT], errors='coerce')
    # 除外内訳をカウント
    count_2 = (payment_amount_numeric == 2).sum()
    count_3 = (payment_amount_numeric == 3).sum()
    count_5 = (payment_amount_numeric == 5).sum()
    ...
    excluded_count = before_count - len(df_filtered)
    # 残存内訳をカウント
    filtered_status = df_filtered.iloc[:, ContractListColumns.ENTRUSTED_STATUS].astype(str).str.strip()
    count_active = (filtered_status == "契約中").sum()
    count_suspended = (filtered_status == "契約中(口振停止)").sum()
    logs.append(f"受託状況フィルタ: {excluded_count}件除外 → {len(df_filtered)}件残存（契約中: {count_active}件, 契約中(口振停止): {count_suspended}件）（最終）")
...
def generate_excel(
    ...
    with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
        for person_type, config in VisitListBackrentConfig.PERSON_TYPES.items():
...
def process_visit_list_backrent(df_input: pd.DataFrame) -> Tuple[bytes, str, str, List[str]]:
    """
    訪問リスト作成（バックレント用）のメイン処理
    ...
    # 3. Excelファイル生成
    today = datetime.now()
    filename = f"{today.strftime('%m%d')}訪問リスト（バックレント用）.xlsx"
    ...
    message = f"訪問リスト（バックレント用）作成完了 - 合計 {total_records}件"
