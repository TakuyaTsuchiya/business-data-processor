...
class TestVisitListFiltering:
    ...
    def setup_method(self):
        ...
        # 委託先法人ID
        data[118] = ['5', None, '10', '5', '', '3']  # 5と空白のみ通す
    ...
    def test_filter_delegated_corp_id_5を含む(self):
        """委託先法人IDが'5'のレコードが含まれることを確認"""
        pass

    def test_filter_delegated_corp_id_空白を含む(self):
        """委託先法人IDが空白（None, ''）のレコードが含まれることを確認"""
        pass

    def test_filter_delegated_corp_id_5以外を除外(self):
        """委託先法人IDが'5'または空白以外のレコードが除外されることを確認"""
        pass
    ...
    def test_filter_combined_すべての条件を満たす(self):
        ...
        # 期待値: 管理番号1001のみ
        # 1001: 交渉困難, 昨日, 10, '5', 東京都 → OK
        # 1002: 死亡決定, 今日, 2（除外される金額）, None
        # 1003: 弁護士介入, 明日（除外される日付）, 5（除外される金額）, '10'（除外される法人ID）, 住所なし
        # 1004: 交渉継続中（除外されるランク）, '5'
        # 1005: 交渉困難, 2日前, 3（除外される金額）, ''
        # 1006: 追跡調査中（除外されるランク）, '3'（除外される法人ID）
