"""
フェイスオートコール緊急連絡人 委託先法人IDフィルタのテスト

ビジネスルール:
- 委託先法人ID: 1, 2, 3, 4, 8 のみ処理対象
- ID 7 は契約者のみで、緊急連絡人は対象外
"""
...
from processors.faith_autocall.emergency_contact.standard import (
    apply_faith_emergencycontact_filters,
)
from domain.rules.business_rules import CLIENT_IDS


class TestFaithEmergencyContactClientIdFilter:
    """フェイス緊急連絡人の委託先法人IDフィルタテスト"""
    ...
    def base_valid_row(self):
        ...
        return {
            ...
            "回収ランク": "通常",
            "滞納残債": "50000",
            "緊急連絡人１のTEL（携帯）": "090-1234-5678",  # 緊急連絡人の電話番号列
            "入居ステータス": "入居中",
            "滞納ステータス": "滞納",
            "管理番号": "TEST001",
            "契約者カナ": "テスト タロウ",
    ...
    def test_client_id_1_is_accepted(self, base_valid_row):
        ...
        result_df, logs = apply_faith_emergencycontact_filters(df)
    ...
    def test_all_valid_client_ids_in_single_batch(self, base_valid_row):
        """全ての有効な委託先法人ID（1,2,3,4,8）が一括で処理される"""
        rows = []
        for client_id in [1, 2, 3, 4, 8]:
        ...
        assert result_ids == {1, 2, 3, 4, 8}

    # === 委託先法人ID 7 は緊急連絡人では除外（契約者のみ対象） ===

    def test_client_id_7_is_rejected_for_emergency_contact(self, base_valid_row):
        """委託先法人ID=7は緊急連絡人では除外（契約者のみ対象）"""
    ...
    def test_client_id_8_is_accepted(self, base_valid_row):
        """委託先法人ID=8は処理対象（新規追加）"""
        ...
        assert len(result_df) == 1
        assert result_df.iloc[0]["委託先法人ID"] == 8
...
class TestBusinessRulesConsistencyEmergencyContact:
    """ビジネスルール定義との整合性テスト（緊急連絡人）"""

    def test_faith_does_not_include_id_7(self):
        """faith（緊急連絡人用）には委託先法人ID 7が含まれていない"""
        assert 7 not in CLIENT_IDS["faith"]

    def test_faith_has_correct_ids(self):
        """faith（緊急連絡人用）が正しいIDリストを持つ（8を含む）"""
        assert set(CLIENT_IDS["faith"]) == {1, 2, 3, 4, 8}

    def test_emergency_contact_uses_faith_not_faith_contract(self):
        """緊急連絡人はfaith_contractではなくfaithを使用すべき"""
