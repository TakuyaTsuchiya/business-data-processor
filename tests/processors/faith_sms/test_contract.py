...
class TestFaithSmsContractIntegration:
    ...
    def test_output_mapping(self, payment_deadline_date):
        """出力マッピングの検証: 電話番号、契約者名、物件名、金額、銀行口座、メモ、支払期限"""
        rows = [
            create_valid_faith_row(
                契約者氏名="山田太郎",
                物件名="サンプルマンション",
...
class TestMixedData:
    ...
    def test_correct_filtering_with_mixed_data(self, payment_deadline_date):
        ...
        result_df, _, _, stats = _run_contract(rows, payment_deadline_date)
