"""
フェイスSMS緊急連絡人プロセッサのテスト

emergency_contact.pyは:
- Filter 6でiloc[:, 56]（BE列）を使用
...
class TestFaithSmsEmergencyContactIntegration:
    ...
    def test_output_mapping(self, payment_deadline_date):
        """出力マッピングの検証: 電話番号はiloc[:, 56]、連絡人=緊急連絡人１氏名、保証人=空"""
        rows = [
            create_valid_faith_row(
                **{
                    "緊急連絡人１氏名": "連絡山田",
        ...
        assert row["支払期限"] != ""
        # 緊急連絡人プロセッサでは連絡人=緊急連絡人１氏名、保証人=空またはNaN
        assert row["連絡人"] == "連絡山田"
...
class TestMixedData:
    ...
    def test_correct_filtering_with_mixed_data(self, payment_deadline_date):
        rows = [
            ...
            # 緊急連絡人TELが固定電話で除外
            create_valid_faith_row(契約者氏名="除外三郎", 緊急連絡人TEL="03-1234-5678"),
            # 通過するデータ（別のID）
            create_valid_faith_row(契約者氏名="通過四郎", 委託先法人ID="8"),
        ]
        result_df, _, _, stats = _run_emergency(rows, payment_deadline_date)
