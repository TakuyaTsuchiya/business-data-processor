...
class TestProcessGuarantorEmergency:
    ...
    # ===========================================
    # 名前2と名前3の両方出力のテスト（新機能）
    # ===========================================

    def test_name2_guarantor_name3_emergency_both_output(self, converter):
        """名前2が保証人、名前3が緊急連絡先の場合、両方出力される"""
        row = pd.Series(
            {
                # 名前2に保証人
                "氏名2": "丹羽進二",
        ...
        assert result["guarantor1"]["カナ"] == "ニワシンジ"
        # 緊急連絡人1には名前3のデータ
        assert result["emergency1"]["氏名"] == "丹羽花子"
        assert result["emergency1"]["カナ"] == "ニワハナコ"
        # 保証人2と緊急連絡人2は空
        assert result["guarantor2"] == {}
        assert result["emergency2"] == {}

    def test_both_emergency_contacts_output_to_1_and_2(self, converter):
        """名前2と名前3が両方緊急連絡先の場合、緊急連絡人1と2に出力される"""
        row = pd.Series(
            {
                # 名前2に緊急連絡先
                "氏名2": "田中一郎",
                ...
                "自宅住所2": "100-0001 東京都千代田区千代田1-1",
                "自宅TEL2": "",
                "携帯TEL2": "090-1111-1111",
                # 名前3にも緊急連絡先
                "氏名3": "田中二郎",
                "氏名3(カナ)": "タナカジロウ",
                "種別/続柄3": "緊急連絡先/兄弟",
                "生年月日3": "1965/05/05",
                "自宅住所3": "100-0001 東京都千代田区千代田1-1",
                "自宅TEL3": "",
                "携帯TEL3": "090-2222-2222",
            }
        )
        ...
        # 緊急連絡人1には名前2のデータ
        assert result["emergency1"]["氏名"] == "田中一郎"
        assert result["emergency1"]["カナ"] == "タナカイチロウ"
        # 緊急連絡人2には名前3のデータ
        assert result["emergency2"]["氏名"] == "田中二郎"
        assert result["emergency2"]["カナ"] == "タナカジロウ"
        # 保証人は空
        assert result["guarantor1"] == {}
        assert result["guarantor2"] == {}

    def test_both_guarantors_output_to_1_and_2(self, converter):
        """名前2と名前3が両方保証人の場合、保証人1と2に出力される"""
        row = pd.Series(
            {
                # 名前2に保証人
                "氏名2": "山田太郎",
                ...
                "自宅住所2": "530-0001 大阪府大阪市北区梅田1-1",
                "自宅TEL2": "",
                "携帯TEL2": "090-3333-3333",
                # 名前3にも保証人
                "氏名3": "山田花子",
                "氏名3(カナ)": "ヤマダハナコ",
                "種別/続柄3": "保証人/配偶者",
                "生年月日3": "1958/07/20",
                "自宅住所3": "530-0001 大阪府大阪市北区梅田1-1",
                "自宅TEL3": "",
                "携帯TEL3": "090-4444-4444",
            }
        )
        ...
        # 保証人1には名前2のデータ
        assert result["guarantor1"]["氏名"] == "山田太郎"
        assert result["guarantor1"]["カナ"] == "ヤマダタロウ"
        # 保証人2には名前3のデータ
        assert result["guarantor2"]["氏名"] == "山田花子"
        assert result["guarantor2"]["カナ"] == "ヤマダハナコ"
        # 緊急連絡人は空
        assert result["emergency1"] == {}
        assert result["emergency2"] == {}

    def test_name2_empty_name3_emergency_goes_to_1(self, converter):
        """名前2が空で名前3が緊急連絡先の場合、緊急連絡人1に出力される（2ではなく1）"""
        row = pd.Series(
            {
                # 名前2は空
                "氏名2": "",
                ...
                "種別/続柄3": "緊急連絡先/父母",
                "生年月日3": "1960/01/01",
                "自宅住所3": "150-0001 東京都渋谷区神宮前1-1",
                "自宅TEL3": "",
                "携帯TEL3": "090-5555-5555",
            }
        )

        result = converter.process_guarantor_emergency(row)

        # 緊急連絡人1には名前3のデータ（空いてる枠から埋める）
        assert result["emergency1"]["氏名"] == "佐藤一郎"
        assert result["emergency1"]["カナ"] == "サトウイチロウ"
        # 緊急連絡人2は空
        assert result["emergency2"] == {}
        # 保証人は空
        assert result["guarantor1"] == {}
