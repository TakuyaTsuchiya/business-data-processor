...
def _show_mirail_menu():
    ...
    if st.button("契約者（10,000円除外）当日約定込み", key="mirail_contract_without10k_today_included", use_container_width=True):
    ...
    if st.button("保証人（10,000円除外）当日約定込み", key="mirail_guarantor_without10k_today_included", use_container_width=True):
...
def _show_faith_menu():
    ...
    # GB新規登録
    st.markdown('<div class="sidebar-category">📋 GB新規登録</div>', unsafe_allow_html=True)
    if st.button("GBガレージバンク新規登録", key="gb_registration", use_container_width=True):
