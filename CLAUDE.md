...
└── capco_registration.py        # カプコ新規登録（電話番号クリーニング機能付き）
```
...
### 📞 カプコ電話番号クリーニング機能
⚠️ **L列「契約者：電話番号」混入文字の自動除去** ⚠️

**問題**: カプコ入力データで電話番号に不要文字が混入（例：`未080-5787-5364`, `×042-361-5460`, `娘080-6868-0817`）
...
**対応パターン**:
- 漢字混入：未、娘、父、母、本人、代理、自宅等
- 記号混入：×、○、※、TEL、携帯等
- ハイフンなし数字：10桁・11桁を適切な形式に変換
- 固定電話・携帯電話の自動判別と正規化

**実装箇所**: `processors/capco_registration.py:293-351`
...
- **絵文字使用禁止**: コード内・コメント内・ログ出力・commit messageで絵文字は一切使用禁止（CP932エンコーディングエラーの原因となる）
