# Business Data Processor v2.1.0-docker

**統合データ処理システム** - Docker化による環境非依存データ処理ツール

## 🌟 概要

複数の業務用データ処理アプリケーションを統合した、**Docker化**されたStreamlit製のWebアプリケーションです。不動産管理業務における契約データの変換・統合処理を自動化し、**どのWindows PC でも簡単に使用可能**な業務効率化ツールです。
...
#### 🏢 ミライルオートコール（6種類）
- **契約者（残債除外）**: 1万円・1万1千円を自動除外
- **契約者（残債含む）**: 残債除外なしで全件処理
...
| ファイル | 用途 | 説明 |
|----------|------|------|
| 🚀起動.bat | 起動 | アプリを起動しブラウザを自動オープン |
| 🛑停止.bat | 停止 | アプリを安全に停止しデータを保護 |
| 🔄再起動.bat | 再起動 | 不具合時の簡単再起動 |
